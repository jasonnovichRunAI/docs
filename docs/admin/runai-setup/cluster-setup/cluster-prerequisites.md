...
summary: This article lists the prerequisites of a cluster installed with Run:ai.
authors:
    -     
date: 2023-Nov-6
---
...
| Tanzu                   | VMWare Kubernetes              | Tanzu supports *containerd* rather than *docker_. See the NVIDIA prerequisites below as well as [cluster customization](customize-cluster-install.md) for changes required for containerd |
...
All are part of the *Kubeflow Training Operator_. Run:ai supports Training Operator version 1.5 and up. To install run:
...
* Using *GPU Utilization*, *Latency* or *Custom metric* will **also** require the Prometheus adapter. The Prometheus adapter is part of the Run:ai installer and can be added by setting the `prometheus-adapter.enabled` flag to `true`. See [Customizing the Run:ai installation](./customize-cluster-install.md) for further information.
